...
def extract_places_via_llm(description: str, ocr_text: str | None) -> list[dict[str, Any]]:
        ...
        if parsed is None:
            ...
            if len(all_values) == 1:
                ...
                for value in all_values:
                    if isinstance(value, list): merged.extend(value)
                    else: merged.append(value)
                parsed = merged

        if isinstance(parsed, dict):
            wrapped = parsed.get("places") or parsed.get("items")
            if isinstance(wrapped, list): parsed = wrapped
            elif isinstance(wrapped, dict): parsed = [wrapped]
            elif parsed.get("name"): parsed = [parsed]
            else: parsed = []
